...
// ================= Livox SDK 선택 분기 =================
#if defined(WITH_LIVOX2) || defined(WITH_LIVOX_SDK2)
  #define USE_LIVOX_SDK2 1
  extern "C" {
    #include "livox_lidar_api.h"   // SDK2 (v1.2.5)
  }
#else
  extern "C" {
    #include "livox_sdk.h"         // SDK1
  }
#endif
...
static void OnData(uint8_t handle, LivoxEthPacket* pkt, uint32_t data_num, void*) {
  ...
  if (seen < kShowFirstPkts && g_first_pkts.compare_exchange_strong(seen, seen+1)) {
    std::printf("[Livox] pkt#%d: handle=%u points=%u type=%u\n",
                seen, handle, data_num, (unsigned)pkt->data_type);
  }
...
#endif // !USE_LIVOX_SDK2

// ================= SDK2 (v1.2.5 헤더에 맞춤) =================
#ifdef USE_LIVOX_SDK2
// (1) Info 콜백: (핸들, 타입, 문자열)
static void OnLidarInfo(uint32_t handle, uint8_t info_type, const char* info, void*) {
  const size_t len = info ? std::strlen(info) : 0;
  std::printf("[SDK2] Info: handle=%u type=%u len=%zu\n",
              handle, (unsigned)info_type, len);
}

// (2) InfoChange 콜백: (핸들, 구조체 포인터)
static void OnLidarInfoChange(uint32_t handle, const LivoxLidarInfo* li, void*) {
  std::printf("[SDK2] InfoChange: handle=%u (info=%p)\n", handle, (const void*)li);
}

// (3) 포인트 콜백
static void OnLidarPoints(uint32_t handle, uint8_t slot, LivoxLidarEthernetPacket* pkt, void*) {
  if (!pkt) return;
  g_pkt_total += 1; g_pts_total += pkt->dot_num;

  int seen = g_first_pkts.load();
  if (seen < kShowFirstPkts && g_first_pkts.compare_exchange_strong(seen, seen+1)) {
    std::printf("[SDK2] pkt#%d: handle=%u slot=%u type=%u dots=%u\n",
                seen, handle, slot, (unsigned)pkt->data_type, (unsigned)pkt->dot_num);
  }

  // ▼ 필요 시 포인트 파싱 (livox_lidar_def.h 포인트 구조체/스케일 확인 후 주석 해제)
  /*
  if (pkt->data_type == kLivoxLidarCartesianCoordinateHighData) {
    const LivoxLidarCartesianHighResPoint* p =
        reinterpret_cast<const LivoxLidarCartesianHighResPoint*>(pkt->data);
    ...
    const LivoxLidarCartesianLowResPoint* p =
        reinterpret_cast<const LivoxLidarCartesianLowResPoint*>(pkt->data);
...
static void InferLoop(cudaStream_t stream) {
  ...
  while (g_running) {
    ...
    if (elems % 4 != 0) { std::fprintf(stderr, "[ERR] elems%%4!=0 (%zu)\n", elems); snap.clear(); continue; }
    ...
    if (has_bad(snap.data(), snap.size())) { std::fprintf(stderr, "[WARN] NaN/Inf drop\n"); snap.clear(); continue; }

    STEP("B: cudaMallocManaged");
...
int main(int argc, char** argv) {
...
#ifdef USE_LIVOX_SDK2
  // --- config 경로 결정: argv[1] > LIVOX_SDK2_CFG > 미정이면 에러 안내
  ...
  if (!cfg || !*cfg) {
    std::fprintf(stderr,
      ...
      "  사용법: ./livox_live <mid360_config.json>\n"
      "  또는   LIVOX_SDK2_CFG=/path/to/mid360_config.json ./livox_live\n");
    return 1;
  }
  std::printf("[SDK2] using config: %s\n", cfg);

  if (!LivoxLidarSdkInit(cfg, "", nullptr)) {
    std::fprintf(stderr, "[SDK2] LivoxLidarSdkInit() failed\n");
    return 1;
  }
  if (!LivoxLidarSdkStart()) {
    std::fprintf(stderr, "[SDK2] LivoxLidarSdkStart() failed\n");
    LivoxLidarSdkUninit();
    return 1;
  }
  ...
  SetLivoxLidarPointCloudCallBack(OnLidarPoints, nullptr);

#else
  // ---------- SDK1 ----------
  if (!Init()) { std::fprintf(stderr, "[Livox] Init() failed\n"); return 1; }
  SetBroadcastCallback(OnDeviceBroadcast);
  SetDeviceStateUpdateCallback(OnDeviceChange);
  if (!Start()) { std::fprintf(stderr, "[Livox] Start() failed\n"); Uninit(); return 1; }
#endif
